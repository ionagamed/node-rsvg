...
describe('Rsvg', function () {
  describe('Rsvg()', function () {
    it('can be constructed with a SVG', async function () {
      // Use buffer.
    ...
    xit('can be a writable stream', async function () {
      // Default constructor.
      var svg = new Rsvg()
      ...
      svg.write('<svg width="4" height="6"></svg>').should.be.true
      await delay(1000)
      ...
      svg.write('<svg width="4" height="6"></svg>').should.be.false
    })

    xit('emits load event when ready - from stream', async function () {
    ...
    xit('emits load event when ready - from svg', async function () {
      var onload = sinon.spy()
      var svg = new Rsvg()
      svg.on('load', onload)
      onload = sinon.spy()
    ...
    it('gives an error for invalid SVG content', async function () {
      ...
      (() => {
        new Rsvg('<svg width="100" height="100"></svg>invalid')
      }).should.throw(/load failure/i)
    })
  })
  ...
  describe('dimensions()', function () {
    ...
    it('gives the size and position of specific elements', async function () {
      const rawSvg = [
        '<svg width="12" height="10">',
        '<rect x="-2" y="3" width="7" height="5" id="r1"/>',
        '<rect x="8" y="4" width="4" height="6" id="r2"/>',
        '<circle cx="8" cy="3" r="3" id="circ"/>',
        '</svg>'
      ].join('\n')
      const svg = new Rsvg(rawSvg)
  ...
  describe('toString()', function () {
    it('gives a string representation', function () {
      const svg = new Rsvg('<svg width="3" height="7"></svg>')
